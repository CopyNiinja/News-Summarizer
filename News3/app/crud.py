...
def get_or_create_publisher(db: Session, name: str,email:str, website: str):
    publisher = db.query(models.Publisher).filter(models.Publisher.name == name).first()
    if publisher is None:
        publisher = models.Publisher(name=name,email=email, website=website)
        db.add(publisher)
        db.commit()
...
def create_image(db: Session, news_id: int, image_url):
    db_image = models.Image(news_id=news_id, image_url=image_url)
    db.add(db_image)
    db.commit()
    db.refresh(db_image)
    return db_image
...
def create_news(db: Session, news: schemas.NewsCreate):
    ...
    publisher = get_or_create_publisher(db, news.news_publisher,f"{news.news_publisher}@example.com", f"https://{news.publisher_website}")
    ...
    for image_url in news.images:
        print("................")
        create_image(db, news_id=db_news.id,    image_url=image_url)
        print("........x........")
